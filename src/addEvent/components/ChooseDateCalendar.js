...
const fetchEvents = async (date, accessToken) => {
  ...
  for (const calendar of calendarData.items) {
    ...
    for (const event of data.items) {
      event.color = calendar.backgroundColor;
    }
...
const CalendarView = ({ color, returnSelectedDate, setEventDate }) => {
  ...
  const renderItem = ({ item }) => {
    ...
    const eventStyle = {};
    if (!isNaN(durationWithinDayInHours)) {
      eventStyle.height = durationWithinDayInHours * hourHeight;
    }
  ...
  return (
    <>
      <CalendarStrip
        ...
        onHeaderSelected={(d1, d2) => {
          calendarRef.current.setSelectedDate(date);
          calendarRef.current.scrollToInitialIndex();
        }}
